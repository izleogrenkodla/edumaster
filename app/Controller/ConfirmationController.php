...
{ 
    ...
    public function beforeFilter()
    {
	  ...
	  public function admin_index()
     {
		...
		$roles = $this->Confirmation->Role->GetRoles();
	  ...
	  public function admin_conform($id) {
		...
		$bsal = $data['User']['BASE_SALARY'];
		 ...
		 if ($this->request->is('post')) {
                    ...
                    $this->Confirmation->save($abc);
                    
                    $this->redirect(array('action' => 'pre_confirmation',$id));
         ...
         $user_data = $this->User->read(null, $id);
		  ...
		  public function admin_pre_confirmation($id)
		  {			  
				$this->layout = 'admin_form_layout';
				$Session_data = $this->Session->read('Auth.Admin');
			...
			if((isset($id)) && ($id)>0)
				{
					$conditions['Confirmation.USER_ID'] = $id;
					$conditions['Confirmation.ROLE_ID'] = $rdata['User']['ROLE_ID'];
				}
			
			 $data = $this->Confirmation->find('all', array(
