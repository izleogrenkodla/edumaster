...
class AdmissionConfirmController extends AppController
{
	...
	public function admin_index(){
		...
		$this->StudentRegistration = ClassRegistry::init('StudentRegistration');
		 ...
		 if(isset($this->params->query["data"]["StudentRegistration"]) && is_array($this->params->query["data"]["StudentRegistration"])) {
          ...
          $this->paginate = array(
            'conditions' => array('INTERVIEW_RESULT'=>1,$conditions),
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->AdmissionConfirm->Validation()) {
                ...
                if ($this->AdmissionConfirm->save($this->request->data)) {
					...
					$this->AdmissionConfirm->saveField("AMOUNT",$this->request->data['AdmissionConfirm']['ADM_FEE']);
					...
					$cls = $abc['StudentRegistration']['ADM_CLASS_ID'];
					
					$this->Fees = ClassRegistry::init('Fees'); 
					
					$fee = $this->Fees->find('first', array(
						'contain' => array(),
						'conditions' => array('CLASS_ID' => $cls)
					));
					
					$tot_fee = $this->Fees->find('all', array(
						'contain' => array(),
						'conditions' => array('CLASS_ID' => $cls,'FEE_TYPE !='=>1,'STATUS'=>1)
					));
					
					$total = 0;
					foreach ($tot_fee as $t)
					{
						$total = $total + $t['Fees']['FEE'];
				...
				$data['Users'] = array(
						...
						'UID_NUMBER' =>$abc['StudentRegistration']['UID_NUMBER'],
						'AADHAR_NUMBER' =>$abc['StudentRegistration']['AADHAR_NUMBER'],
                    );
					...
					$pk = $this->Users->getLastInsertId();
					
					
					$this->FeeDue = ClassRegistry::init('FeeDue');
				
				$this->MonthDistribution = ClassRegistry::init('MonthDistribution');
							
								$month = $this->MonthDistribution->find('all', 
											  array('conditions' => array('payment_type' => $this->request->data['AdmissionConfirm']['PAYMENT_TERM']), 
												'order' => array('payment_type' => 'DESC') ));
								
								if(sizeof($month)>0) {
								
									foreach($month as $key => $value){
										 //PR($value);
										 $monthArray[$key] = $value['MonthDistribution']['DISTRI_ID'];
									}
				
				
				if($this->request->data['AdmissionConfirm']['PAYMENT_TERM'] == 1)
				{
					
									foreach($monthArray as $kp)
									{
										...
										'Total_Fees' => $total,
										'Status' => 1,
										'Fees' => $this->request->data['Fee'],
										'Month' => $kp,
										'PaymentType' => $this->request->data['AdmissionConfirm']['PAYMENT_TERM'],
										);
										
									$this->FeeDue->create();
									$this->FeeDue->save($FeeDue);
									
									}
				}elseif($this->request->data['AdmissionConfirm']['PAYMENT_TERM'] == 2)
				{
				...
				}elseif($this->request->data['AdmissionConfirm']['PAYMENT_TERM'] == 3)
				{
				...
				}elseif($this->request->data['AdmissionConfirm']['PAYMENT_TERM'] == 4)
				{
									foreach($monthArray as $kp)
									{
											$FeeDue['FeeDue'] = array(
				...
				$this->AdmissionFee = ClassRegistry::init('AdmissionFee');
							   
				  $fee1['AdmissionFee'] = array(
					'USER_ID' => $pk,
					'CLASS_ID' => $abc['StudentRegistration']['ADM_CLASS_ID'],
					'FEE_DATE' => $this->General->datefordb(date('Y/m/d')),
					'ADM_FEE' => $this->request->data['AdmissionConfirm']['ADM_FEE'],
					'REMARK' => $this->request->data['AdmissionConfirm']['REMARK'],
					);
					
							   
				  $this->AdmissionFee->create();
                  $this->AdmissionFee->save($fee1);
				  
				  $this->StudentLedger = ClassRegistry::init('StudentLedger');
				  
				   $StudentLedger['StudentLedger'] = array(
					'USER_ID' => $pk,
					'CLASS_ID' => $abc['StudentRegistration']['ADM_CLASS_ID'],
					'FeesTerms' => $this->request->data['AdmissionConfirm']['PAYMENT_TERM'],
					'Total_Fees' => $total,
					'Paid_Fees' => 0,
					'Remaining' => $total,
					);
				  ...
				  $this->StudentLedger->create();
                  $this->StudentLedger->save($StudentLedger);

                    $this->Session->setFlash('Admission Fee Paid Successfully!', 'message_good');
                    $this->redirect(array('action' => 'receipt',$admid));
                }
				} else {
					$this->Session->setFlash('Admission Fee Paid Please Try Again!', 'message_bad');
				}
			...
			$fee = $this->Fee->find('first', array(
                'contain' => array(),
                'conditions' => array('CLASS_ID' => $abc['StudentRegistration']['ADM_CLASS_ID'])
            ));
			
			$fee_det = $fee['Fee']['FEE'];
			
			$this->set('fee_det',$fee_det);
			
			$this->Fee = ClassRegistry::init('Fee');
			$tot_fee = $this->Fee->find('all', array(
						'contain' => array(),
						'conditions' => array('CLASS_ID' => $abc['StudentRegistration']['ADM_CLASS_ID'],'FEE_TYPE !='=>1)
					));
					
					$total = 0;
					foreach ($tot_fee as $t)
					{
						$total = $total + $t['Fee']['FEE'];
					
					}
					
			$this->set('total_fee',$total);
			
			$this->set('udocumaent',$abc);
		
		$this->PaymentType = ClassRegistry::init('PaymentType');
        /*$payment_terms = $this->AdmissionConfirm->PaymentType->getPaymentTerms();*/
	...
	public function admin_GetStatus($id = null){
	 ...
	 public function admin_donationlist() {
        ...
        if (empty($Session_data)) {
		 ...
		 $std_name = $this->AdmissionConfirm->StudentRegistration->GetName();
         $this->set('student', $std_name);
		
	 $res =$this->AdmissionConfirm->find('all', array(
        ));
		$this->set('AdmissionConfirm',$res);
	  ...
	  public function checkUniqueId($id = null, $no_user)
    {
	 ...
	 public function send_mail($password,$username) {
		 $role = 'Student';
		 $this->StudentRegistration = ClassRegistry::init('StudentRegistration');
					$abc = $this->StudentRegistration->find('first', array(
						'contain' => array(),
        ...
        $body = str_replace(
            ...
            array($abc['StudentRegistration']['FIRST_NAME'].' '.$abc['StudentRegistration']['MIDDLE_NAME'].' '.$abc['StudentRegistration']['LAST_NAME'],
			$abc['StudentRegistration']['USERNAME'],$abc['StudentRegistration']['PASSWORD'],ADMIN_URL, WEBSITE_NAME,$role_name), $message);
	...
	public function admin_GetFee(){
		...
		if($p_t == 1)
		{
		 ...
		 echo  "<div class='col-md-6' id='Fee'>
						<div class='form-group' >
						<label class='control-label col-md-3'>Fee</label><div class='col-md-9'>
						<input type='text' name='Fee' class='form-control' value='$fee' readonly/>
						</div></div></div>";
		die;
	}
