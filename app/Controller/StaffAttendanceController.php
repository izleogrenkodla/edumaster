    ...
    $Session = $this->Session->read('Auth.Admin');
	 ...
	 if(isset($this->params->query["data"]["User"]) && is_array($this->params->query["data"]["User"])) {
            ...
            if((isset($post["from_date"]) && $post["from_date"]!='') && (isset($post["to_date"]) && $post["to_date"]!='') ) {
                $conditions['StaffAttendance.ATTENDANCE_DATE BETWEEN ? AND ?'] = array($this->General->datefordb($post["from_date"]),$this->General->datefordb($post["to_date"]));
            }
        }
		...
		$this->User = ClassRegistry::init('User');
			...
			if((isset($id)) && ($id)>0)
				{
					$conditions['StaffAttendance.USER_ID'] = $id;
					$conditions['StaffAttendance.ROLE_ID'] = $rdata['User']['ROLE_ID'];
				}
           ...
           $this->StaffAttendance->set($this->request->data);
              ...
              //  $this->TeacherTimeTable->create();
                ...
                if ($this->request->data) {
					 if ($this->StaffAttendance->Validation()) {
                    ...
                    $this->request->data['StaffAttendance']['USER_ID'] = $Session_data['ID'];
                      ...
                      foreach($list as $key => $stid)
                        {
                            $status = $stid;
                             ...
                             $ttdata['StaffAttendance'] = array(
                                        'STATUS'=>$status,
                                        'ATTENDANCE_DATE'=>$this->General->datefordb($this->request->data['StaffAttendance']['ATTENDANCE_DATE']),
                                        'ROLE_ID'=>$role_id,
                                        'USER_ID'=>$user_id,
