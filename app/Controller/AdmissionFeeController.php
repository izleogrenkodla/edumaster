...
class AdmissionFeeController extends AppController
{
    var $name = 'AdmissionFee'; 
	...
	public function admin_index(){
		...
		if(isset($this->params->query["CLS"]) && ($this->params->query["CLS"]>0)) {
            $conditions["AdmissionFee.CLASS_ID"] = $this->params->query["CLS"];
            $this->request->data["AdmissionFee"]["CLS"] = $this->params->query["CLS"];
        }
		
		  $this->layout = 'admin_form_layout';
          $this->paginate = array(
            'conditions' => $conditions,
            'Contain' => array('Name','AcademicClass'),
            'limit' => PAGINATION_LIMIT_ADMIN,
            'order' => 'AdmissionFee.ADM_FEE_ID DESC'
        );
		
        $this->set('AdmissionFee', $this->paginate('AdmissionFee'));
		
		 $classes = $this->AdmissionFee->AcademicClass->GetAcademicClasses();
