<?php
class FeesRefundController extends AppController
{
    var $name = 'FeesRefund';
    ...
    public function beforeFilter()
    {
        ...
        $this->Auth->allow('App_TotalFeesRefund','App_TotalPaidFeesRefund','App_TotalRemainingFeesRefund');
    ...
    public function admin_index()
    {
        ...
        $this->FeesRefund->recursive = 0;
        $this->paginate = array(
            'conditions' => '',
            'limit' => PAGINATION_LIMIT_ADMIN,
            'order' => 'FeesRefund.created ASC'
        );

        $this->set('FeesRefund', $this->paginate('FeesRefund')); 
    }
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Fee->Validation()) {
                ...
                if ($this->Fee->save($this->request->data)) {
                    ...
                    $this->Session->setFlash('FeesRefund Added Successfully!', 'message_good');
                    $this->redirect(array('action' => 'index'));
                }
            } else {
                $this->Session->setFlash('FeesRefund Not Added Please Try Again!', 'message_bad');
            }
        }
        ...
        if ($this->request->is('put') || $this->request->is('post')) {
            if ($this->Fee->Validation()) {
                if ($this->Fee->save($this->request->data)) {
                   ...
                   $this->Session->setFlash('FeesRefund Updated Successfully!', 'message_good');
                    $this->redirect(array('action' => 'index'));
                } else {
                    $this->Session->setFlash('FeesRefund Not Saved Please Try Again!', 'message_bad');
                }
    ...
    {
        $FeesRefund = $this->Fee->find('first', array(
            'contain' => array(),
            'conditions' => array('FEE_ID' => $Id)
        ));
    ...
    public function App_TotalFeesRefund()
    {
        $this->layout = 'ajax';
        ...
        $FeesRefund = $this->Fee->find('all', array(
    ...
    public function App_TotalPaidFeesRefund()
    {
        $this->layout = 'ajax';
        $this->autoRender = false;
        $message = '';
        ...
        $FeesRefund = $this->LedgerXref->find('all', array(
    ...
    public function App_TotalRemainingFeesRefund()
    {
        $this->layout = 'ajax';
        $this->autoRender = false;
        $message = '';
        ...
        $tmp = array();
          $j = array();
        foreach($FeesRefund as $key => $val)
        {
            $feeType = $val['Fee']['FEE_TYPE'];
            $feeAmount =  $val['Fee']['FEE'];

            $PaidFeesRefund = $this->LedgerXref->find('all', array(
                'contain' => array('FeeType'),
                'conditions' => array('LedgerXref.USER_ID' => $UserId,'LedgerXref.FEES_TYPE'=>$feeType, 'LedgerXref.STATUS' => 1)

            ));
            if(isset($PaidFeesRefund) && sizeof($PaidFeesRefund))
            {
               foreach($PaidFeesRefund as $paidkey=>$paidvalue ) {
        ...
        if(isset($j) && !empty($j))
        {
            $message = 'Total Remaining FeesRefund';
            $status = true;
	    $newtmp[] = $j;
            $result_array = array(
                'status' => $status,
