     ...
     public function admin_add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Homework->Validation()) {
				...
				$img = '';
					if(isset($this->request->data["Homework"]["UPLOAD_IMAGE"]["size"]) && $this->request->data["Homework"]["UPLOAD_IMAGE"]["size"]>0) {
						$img = $this->request->data["Homework"]["UPLOAD_IMAGE"];
						unset($this->request->data["Homework"]["UPLOAD_IMAGE"]);
					}
				
                if ($this->Homework->save($this->request->data)) {
					
					$lastid = $this->Homework->getLastInsertId();
                        ...
                        $fname = HW.strtotime(date('Y-m-d H:i:s')).$lastid.'.'.$ext[1];
                        move_uploaded_file($img["tmp_name"],$path.$fname);

                        $im = file_get_contents($path.$fname);
                      
                        $this->Homework->id = $lastid;

                        $this->Homework->saveField("IMAGE_URL",$fname);
    ...
    public function admin_edit($id = null)
    {
   	...
   	public function send_mail_to_student($post=array()) {
        ...
        $body = str_replace(
            ...
            array($post['STUDENT_NAME'],$post['HW_MESSAGE'],$post['TEACHER_STATUS'],ADMIN_URL, WEBSITE_NAME), $message);
    ...
    public function admin_delete($Id = null)
    {
        ...
        if (!empty($Id)) {
            ...
            try {
				...
				$UserData = $this->Homework->find('first', array(
                'contain' => array(),
                'conditions' => array('HW_ID' => $Id)
				));
				
				
				$fileName = $UserData['Homework']['IMAGE_URL'];
                ...
                if ($this->Homework->delete($Id, false)) {
