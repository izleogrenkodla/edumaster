...
<tr role="row" class="heading">
	...
	<?php if($authUser["ROLE_ID"]==SUPERVISOR_ID || $authUser["ROLE_ID"]==ADMIN_ID) { ?>
    ...
    <?php foreach($exams as $exm) { ?>
        <tr>
			...
			<?php if($authUser["ROLE_ID"]==SUPERVISOR_ID) { ?>
                ...
                ))?>" class="tooltips btn" data-toggle="tooltip" data-placement="top" title="Generate Result [<?php echo $exm['AcademicClass']['CLASS_NAME']; ?>]">
                    <i class="fa fa-check-square"></i></a>
			<?php  } ?>
					  <?php if($authUser["ROLE_ID"]==ADMIN_ID) { ?>
					<a href="<?php echo Router::url(array('action' => 'edit', $exm['Exam']['EX_ID'],
					))?>" class="tooltips btn" data-toggle="tooltip" data-placement="top" title="Edit Result">
						<i class="fa fa-pencil"></i></a>
					<a href="javascript:void(0)" onclick="remove_record('<?php echo Router::url(array('action' => 'delete', $exm['Exam']['EX_ID']))?>')" class="tooltips btn" data-toggle="tooltip" data-placement="top" title="Delete Result">
