...
<?php if(in_array($authUser["ROLE_ID"],array(TRANSPORTATION_ID))) { ?>
	...
	<a href="<?php echo Router::url(array('controller' => 'VehicleRoute','action' => 'add')) ?>" class="btn
			green bg-green"> ADD NEW <i class="fa fa-plus"></i>
	</a>
<?php }?>
...
<?php if(count($VehicleRoute) > 0):
    ...
    <?php foreach($VehicleRoute as $vehicleroute) { ?>
        <tr>
            ...
            <td class="text-center"><?php echo $vehicleroute['VehicleShift']['VEHICLE_SHIFT_TYPE']; ?></a></td>
            <td class="text-center"><?php echo $vehicleroute['Vehicle']['VEHICLE_NUMBER']; ?></a></td>
            ...
            <td class="text-center">
                <a href="<?php echo Router::url(array('controller' => 'vehicleroute',
                    ...
                    <a href="javascript::void(0);" onclick="remove_record('<?php echo Router::url(array('controller' => 'VehicleRoute','action' => 'delete',  $vehicleroute['VehicleRoute']['ROUTE_RELATION_ID']))?>')" class="tooltips btn" data-placement="top" data-toggle="tooltip" title="Delete">
