						...
						<div class="col-md-6">
							...
							<div class="form-group">
                                ...
                                <div class="col-md-9 tooltips" data-container='body' data-placement='bottom'
                                     data-html='true' data-original-title="First Name">
									<?php
										echo $this->Form->input('CIR_GR_ID', array('options' => $CircularGroup ,'disabled', 'default' => '','onchange' => 'test()', 'class' => 'form-control select2me','label' => FALSE_VALUE,'div' => FALSE_VALUE));
                                    ?>
...
function noticeboard() { 
		...
		if($("#NoticeBoardROLEID").val()==0) {
			...
			$("#NoticeBoardCIRGRID").attr('disabled', 'disabled');
			document.getElementById("users_list").innerHTML = "";
			$("#checkall").hide();
			alert("Please select Valid Role");
			
			
		}else if($("#NoticeBoardROLEID").val()== <?php echo STUDENT_ID;?> && $("#NoticeBoardCIRGRID").val()==0){
			document.getElementById("users_list").innerHTML = "";
			$("#checkall").hide();
			$("#NoticeBoardCIRGRID").removeAttr("disabled");
			return false;
