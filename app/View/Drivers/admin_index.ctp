...
<?php if(count($drivers) > 0): ?>
    <?php foreach($drivers as $key=>$driver) { ?>
        <tr>
            ...
            <td>
			<?php if(in_array($authUser["ROLE_ID"],array(TRANSPORTATION_ID))){?>
			<a href="<?php echo Router::url(array('controller' => 'Drivers', 'action' => 'edit', $driver['Driver']['DRIVER_ID'],))?>">
			<?php } ?>
			<?php echo $this->General->first_letter_capitalized($driver['Driver']['FIRST_NAME']. ' '.$driver['Driver']['MIDDLE_NAME']. ' '.$driver['Driver']['LAST_NAME']); ?>
			<?php if(in_array($authUser["ROLE_ID"],array(TRANSPORTATION_ID))){?>
			</a>
			<?php } ?>
			</td>
