...
if(count($StoreReceivRequest) > 0):
    ...
    <?php foreach($StoreReceivRequest as $lg) { ?>
        <tr>
			...
			<?php if(in_array($authUser["ROLE_ID"],array(STORE_ID))){?>
            <td class="text-center">
			 <a href="javascript::void(0);" onclick="update('<?php echo Router::url(array('controller' => 'StoreDistribution','action' => 'add', $lg['StoreReceivRequest']['ID']))?>')" class="tooltips btn" data-toggle="tooltip" data-placement="top" title="Distribute">
...
<script>
function update(x) {
	if(confirm("Are you sure want to distribute this item ?")) { 
		window.location.href = x;
	}
}
</script> 