        ...
        if ($this->request->is('put') || $this->request->is('post')) {
            if ($this->School->Validation()) {
                ...
                if($this->request->data["School"]["UPLOAD_BROCHURE"]["size"]>0) {
				 ...
				 $sing = '';
                if($this->request->data["School"]["AUT_SIGN"]["size"]>0) {
                    $sing = $this->request->data["School"]["AUT_SIGN"];
                    unset($this->request->data["School"]["AUT_SIGN"]);
                }
                ...
                if ($this->School->save($this->request->data)) {
                    ...
                    if(is_array($logo) && $logo["size"]>0) {
					  ...
					  if(is_array($sing) && $sing["size"]>0) {
                        ...
                        $path = WWW_ROOT . "/files/upload_document/";
                        $fname = strtotime(date('Y-m-d H:i:s')).$id.'.'.$ext[1];
                        move_uploaded_file($sing["tmp_name"],$path.$fname);

                        $fileName = $SchoolData['School']['AUT_SIGN'];
                        ...
                        $this->School->saveField("AUT_SIGN",$fname);
                    }
