...
<?php if(count($StaffUploadDocument) > 0):
    ...
    <?php foreach($StaffUploadDocument as $udoc) { ?>
        <tr>
			...
			<?php if(in_array($authUser["ROLE_ID"],array(HR_ID))) { ?>
            <td class="text-center">
            	<a href="javascript:void(0)" onclick="remove_record('<?php echo Router::url(array('controller' => 'StaffUploadDocument','action' => 'delete', $udoc['StaffUploadDocument']['UPL_DOC_ID']))?>')" class="tooltips btn" data-container="body" data-placement="top" data-html="true"
